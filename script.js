...
// ===== SYSTEM SETTINGS UPDATE (NEW) =====
function updateSystemSettings() {
    // 1. Get Values
    config.accent = document.getElementById('cfgAccent').value;
    config.bg = document.getElementById('cfgBg').value;
    config.ratio = document.getElementById('cfgRatio').value;

    // 2. Apply to CSS Variables in Editor
    document.documentElement.style.setProperty('--accent-primary', config.accent);
    document.documentElement.style.setProperty('--accent-glow', config.accent); // Simplification: make glow same color
    document.documentElement.style.setProperty('--bg-dark', config.bg);

    // 3. Apply Ratio to Preview
    document.getElementById('previewWrapper').style.aspectRatio = config.ratio;
    
    // 4. Update Border colors in editor
...
// ===== THE EXPORTER (GENERATES THE SOLO LEVELING STYLE HTML WITH MARKER) =====
function generateAndExport() {
    const slidesJson = JSON.stringify(slides);
    
    // Inject Custom Settings into generated CSS
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        body {
            ...
            background-image: radial-gradient(circle at 50% 50%, rgba(255,255,255,0.05) 0%, var(--bg) 100%);
            height: 100vh;
            color: #e0e0e0;
            overflow: hidden;
            display: flex;
...
<body>
    ...
    <script>
        ...
        // --- TOOLS LOGIC (PEN + MARKER + LASER) ---
        const canvas = document.getElementById('drawingCanvas');
        const ctx = canvas.getContext('2d');
        let drawing = false;
        let activeTool = null; // 'pen', 'marker', 'laser', null
        
        function resize() { canvas.width = window.innerWidth; canvas.height = window.innerHeight; }
        window.onresize = resize; resize();

        function toggleTool(tool) {
            // If clicking same tool, toggle off
            if (activeTool === tool) {
                activeTool = null;
        ...
        canvas.addEventListener('mousedown', e => { 
            if(activeTool === 'pen' || activeTool === 'marker') { 
                ...
                if (activeTool === 'pen') {
                    ...
                    ctx.globalCompositeOperation = 'multiply'; // Mixes color
