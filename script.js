...
function updateSlideData() {
    ...
    // Optimizing: only re-render list title if changed, but for simplicity re-render all
...
// ===== THE EXPORTER (GENERATES THE SOLO LEVELING STYLE HTML) =====
function generateAndExport() {
    const slidesJson = JSON.stringify(slides);
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        body {
            ...
            background-image: radial-gradient(circle at 50% 50%, #1a0b2e 0%, #0b0c15 100%);
            height: 100vh;
            color: #e0e0e0;
            overflow: hidden;
            display: flex;
        ...
        body.drawing-mode #drawingCanvas { pointer-events: auto; cursor: crosshair; }
        body.laser-active { cursor: none; } /* Hide default cursor for laser */
        ...
        .slide {
            ...
            box-shadow: 0 0 50px rgba(0,0,0,0.8), 0 0 20px rgba(123, 44, 191, 0.3);
            border-radius: 5px;
            padding: 60px;
            display: none;
            position: relative;
        ...
        /* TYPOGRAPHY */
        h1 { color: #9d4edd; font-size: 3.5rem; text-transform: uppercase; border-bottom: 2px solid #7b2cbf; padding-bottom: 20px; margin-bottom: 30px; text-shadow: 0 0 10px rgba(157, 78, 221, 0.5); letter-spacing: 2px; }
        ...
        img { max-width: 100%; border: 1px solid #7b2cbf; box-shadow: 0 0 15px rgba(0,0,0,0.5); }
        ...
        .btn.active { color: #00b4d8; text-shadow: 0 0 8px #00b4d8; background: rgba(0, 180, 216, 0.1); }
        .btn.pen-active { color: #ff0055; text-shadow: 0 0 8px #ff0055; }
        
        .page-counter { color: #7b2cbf; font-weight: bold; font-size: 18px; font-family: monospace; }
...
<body>
    ...
    <div class="controls-overlay">
        ...
        <div class="control-panel">
            <button class="btn" id="btnLaser" onclick="toggleLaser()"><i class="fas fa-crosshairs"></i></button>
            <button class="btn" id="btnPen" onclick="togglePen()"><i class="fas fa-pen"></i></button>
    ...
    <script>
        ...
        const canvas = document.getElementById('drawingCanvas');
        const ctx = canvas.getContext('2d');
        let drawing = false, penMode = false, laserMode = false;
        ...
        function togglePen() {
            penMode = !penMode;
            laserMode = false;
            document.body.classList.toggle('drawing-mode', penMode);
            document.body.classList.remove('laser-active');
            document.getElementById('btnPen').classList.toggle('pen-active', penMode);
            document.getElementById('btnLaser').classList.remove('active');
        ...
        function toggleLaser() {
            ...
            penMode = false;
            document.body.classList.remove('drawing-mode');
            document.body.classList.toggle('laser-active', laserMode);
            document.getElementById('btnLaser').classList.toggle('active', laserMode);
            document.getElementById('btnPen').classList.remove('pen-active');
        }

        function clearCanvas() { ctx.clearRect(0,0,canvas.width, canvas.height); }

        // Draw Events
        canvas.addEventListener('mousedown', e => { if(penMode) { drawing=true; ctx.beginPath(); ctx.moveTo(e.clientX, e.clientY); } });
        window.addEventListener('mouseup', () => drawing=false);
        canvas.addEventListener('mousemove', e => {
            if(penMode && drawing) {
                ctx.lineWidth = 4; ctx.strokeStyle = '#ff0055'; ctx.shadowBlur = 10; ctx.shadowColor = '#ff0055';
